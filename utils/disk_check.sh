...
# --- Configuration ---
# Set the partition/filesystem to monitor (e.g., /lfs/h2/emc/obsproc/noscrub)
PARTITION="/lfs/h2/emc/stmp/iliana.genkova"
...
if [ "$USAGE" -ge "$THRESHOLD" ]; then
    # Define the email content
    SUBJECT="CRITICAL: DEV /lfs/h2/emc/stmp/iliana.genkova Disk Space Alert - $PARTITION at ${USAGE}%"
    BODY="The disk partition $PARTITION is at ${USAGE}% usage. Please take action immediately."

    # Send the email using the 'mail' command (common on Linux/UNIX systems)
    echo "$BODY" | mail -s "$SUBJECT" "$ADMIN"
    echo "$BODY" | mail -s "$SUBJECT" catherine.Thomas@noaa.gov
fi
...
# --- Configuration ---
# Set the partition/filesystem to monitor (e.g., /lfs/h2/emc/obsproc/noscrub)
PARTITION="/lfs/h2/emc/ptmp/iliana.genkova"
...
if [ "$USAGE" -ge "$THRESHOLD" ]; then
    # Define the email content
    SUBJECT="CRITICAL: DEV /lfs/h2/emc/ptmp/iliana.genkova Disk Space Alert - $PARTITION at ${USAGE}%"
    BODY="The disk partition $PARTITION is at ${USAGE}% usage. Please take action immediately."
