...
# Configuration
FILE_TO_CHECK="/lfs/h1/ops/prod/dcom/${PDY}/b021/xx045"  # <--- **CHANGE THIS PATH**
EMAIL_RECIPIENT="iliana.genkova@noaa.gov" # <--- **CHANGE THIS EMAIL ADDRESS**
TIME_LIMIT_MINUTES="30" #15 #Check if updated in the last TIME_LIMIT_MINUTES minutes
...
if [ ! -f "$FILE_TO_CHECK" ]; then
    ...
    mail -s "ALERT: b021/xx045 File Check Failed - File Not Found" "$EMAIL_RECIPIENT" <<< "The script attempted to check the file '$FILE_TO_CHECK' but it does not exist."
    exit 1
fi
...
# Configuration
FILE_TO_CHECK="/lfs/h1/ops/prod/dcom/${PDY}/b005/xx030"  # <--- **CHANGE THIS PATH**
EMAIL_RECIPIENT="iliana.genkova@noaa.gov" # <--- **CHANGE THIS EMAIL ADDRESS**
TIME_LIMIT_MINUTES="60" #15 #Check if updated in the last TIME_LIMIT_MINUTES minutes
...
if [ ! -f "$FILE_TO_CHECK" ]; then
    echo "Error: File not found at $FILE_TO_CHECK"
    # Send an alert about the missing file as well
    mail -s "ALERT: b005/xx030 File Check Failed - File Not Found" "$EMAIL_RECIPIENT" <<< "The script attempted to check the file '$FILE_TO_CHECK' but it does not exist."
    exit 1
fi
