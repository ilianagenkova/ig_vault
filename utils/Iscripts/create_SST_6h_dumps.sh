...
mkdir -p $out_dir/gdas.${PDY}/${cyc}/ocean/sst
...
# remove the --dry-run partial test at some point !
...
if [ $cyc -eq 00 ]; then
  rsync -avhWr --no-compress --progress --include-from=all_includes --exclude-from=all_excludes $DCOMROOT/${PDYm1}/sst/ $out_dir/gdas.${PDY}/${cyc}/ocean/sst
fi
  rsync -avhWr --no-compress --progress --include-from=all_includes --exclude-from=all_excludes $DCOMROOT/${PDY}/sst/ $out_dir/gdas.${PDY}/${cyc}/ocean/sst
